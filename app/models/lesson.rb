class Lesson < ApplicationRecord
  ...
  def check_if_users_have_lessons_at_time_range
    sender_recurring_lessons = sender.lessons.where(recurring: true).map do |l|
      l.starts_at = DateTime.parse("#{weekday(l.starts_at)} #{l.starts_at.to_s(:time)}")
      l.ends_at = DateTime.parse("#{weekday(l.ends_at)}, #{l.ends_at.to_s(:time)}")
      l
    end
  ...
  def check_if_users_are_available
    ...
    receiver_available_from = Time.parse(receiver.availability[lesson_day][:from]).to_time
    receiver_available_to = Time.parse(receiver.availability[lesson_day][:to]).to_time
    ...
    sender_available_from = Time.parse(sender.availability[lesson_day][:from]).to_time
    sender_available_to = Time.parse(sender.availability[lesson_day][:to]).to_time
    errors.add(:starts_at,
  ...
  def check_if_time_range_is_valid
    ...
    unless starts_at > DateTime.now || recurring
